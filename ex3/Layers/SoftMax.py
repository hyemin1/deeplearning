...
class SoftMax(Base.BaseLayer):
    ...
    def forward(self, input_tensor):
        ...
        reduced_sum = np.sum(np.array(reduced_exp), axis=1, keepdims=True)
        #reduced_sum = np.sum(reduced_exp, axis=1)
    ...
    def backward(self, error_tensor):
        ...
        temp_sum = np.multiply(error_tensor, self.output_tensor)

        # temp_sum=np.dot(np.asmatrix(error_tensor),np.asmatrix(self.output_tensor))
        sum_by_rows = np.sum(np.array(temp_sum).T, axis=0, keepdims=True)
        ...
        prev_error_tensor = np.multiply(self.output_tensor,error_tensor)
