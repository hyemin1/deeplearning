                ...
                for ker in range(self.num_kernels):
                    for r in range(error_tensor.shape[2]):
                        for c in range(error_tensor.shape[3]):
                            if (r * self.stride_shape[0] < upsampled_error.shape[2] and c * self.stride_shape[1] <
                                    upsampled_error.shape[3]):
                                upsampled_error[batch, ker, r * self.stride_shape[0], c * self.stride_shape[1]] = \
                                    error_tensor[batch, ker, r, c]
                            else:
                                print("not found")
        else:
            upsampled_error = np.zeros(
